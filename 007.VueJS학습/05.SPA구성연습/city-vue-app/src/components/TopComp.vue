...
<template>
  <header>
    ...
    <!-- 메뉴선택이동링크 -->
    <div class="m2">
      <!-- v-on:이벤트명.prevent 기본기능막기 옵션 .prevent는 event.preventDefault()와 같다!
        참고) v-on:이벤트명.stop 이벤트버블링막기 옵션 .stop은 event.stopPropagation()과 같다! -->
...
export default {
  // 컴포넌트를 부르는 이름
  name: "TopArea",
  // 데이터 셋업 - 내부 리턴 필수 : 리턴함수로 만든다
  data: function () {
    return {
      // 1. 도시정보 객체변수
      sdata: this.$store.state.cityData,
      // 2. 메뉴번호 (처음에 다음메뉴인 2번 넣기)
      num: 2,
    };
  }, // data 구역
  methods: {
