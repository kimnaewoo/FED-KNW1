...
// 요구사항1 : 원형 SVG느 각각 %를 다르게 하여 숫자% 와 함께 진행율을 애니메이션 하여 표현한다.
...
// 2. 이벤트 설정
// 대상.click() 는 계속 이벤트 적용되므로,
// 대상.one('click',함수) 로 한번만 이벤트 적용하는 메서드 
// 제이쿼리는 내부적으로 여러요소는 개별적으로 for문으로 셋팅하므로 forEach같은 제어를 할 필요가 없다.
...
function progFn(seq,max){ 
    ...
    /* 
        수치계산법 : 
        전체값 * (100 - 현재% 수치)/100 = 원하는값 
        -> 300에서부터 거꾸로 작아져야함 
        -> 퍼센트 수치를 큰값에서 작은값으로 가도록한다.
        [ 100% - 현재% 수치 ]
    */
