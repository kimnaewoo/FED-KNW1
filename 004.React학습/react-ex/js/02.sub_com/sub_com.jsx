...
function 이야기(){
    ...
    return (
        <div style={{
            ...
            {/* 현재산빼고 나머지산 버튼만들기 : 이버튼 클릭시 메인컴포넌트의 상태훜 변수가 업데이트되어 전체가 변경됨! 
                -> changeMyVal() 메서드 사용 컨텍스트 프로바이더에서 제공함 */}
            <div>
                {
                    btnData.map(v=>
                    <button onClick={()=>맘대로.changeMyVal(v.이름)} style={{padding:'15px',fontSize:'20px',margin:'10px'}} >{v.이름}
                    </button>)
                }
            </div>
...
function MyImg(){
    ...
    React.useEffect(()=>{
        ...
        return(()=>{
            console.log('나는 소멸한다...!');
            // 소멸하여 이미지가 안나오는 산은 "에베레스트" 이므로 메시지를 띄어준다!
            alert('에베레스트는 세계에서 가장 높은 산입니다!');
        })
