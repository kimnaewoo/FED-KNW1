// 07. 리액트 : 조건렌더링 + 리스트 렌더링 

/*************************************************************************************************** 
    1. if문을 사용하여 조건부 렌더링하기
    - 리액트에서는 조건부로 구성요소를 렌더링할수 있다!
***************************************************************************************************/
...
// 컴포넌트 호출하기1 : 개발자찍기
ReactDOM.render(<Developer isDev={true} isrc={devImg[0]} ialt='개발자 공유' itit='프론트엔드 개발자 공유입니다!' />,document.querySelector('#root1'));

// 컴포넌트 호출하기2 : 비개발자찍기
ReactDOM.render(<Developer isDev={false} isrc={devImg[1]} ialt='주먹왕 마동석' itit='개발자가 뭡니까..?!' />,document.querySelector('#root2'));
...
function FoodList(props){ // 음식명은 fname에 담아서 전달한다!
    return <li>개발자는 {props.fname} 좋아해요!</li>
} // FoodList 컴포넌트
...
function WishList2(props){
    ...
    return(
        <React.Fragment>
            ...
            {
                mymv.length == 0 &&
                <h2>아직 개발자 영화 리스트가 없데이트 되지 않았습니다.</h2>
            }
        </React.Fragment>
    );
} // WishList2 컴포넌트
...
// 4-2. 전체 출력 컴포넌트
// 구성 : 전체타이틀(Title컴포넌트사용) + 변경버튼 + 작가와 그림출력(MakeWork컴포넌트)
...
function ExpComp(props){
    ...
    return(
        <React.Fragment>
            ...
            {/* 작품출력: 삼항연산자로 작품변경하기 
            result의 변수를 후크변수로 셋팅하면 컴포넌트가 이 변수의 값이 변경됨에 따라 자동으로 재설정된다. */}
            {
                result ?
                <MakeWork painter='피카소' wname='우는여인' /> : <MakeWork painter='모네' wname='양산을 쓴 여인' /> 
            }
        </React.Fragment>
    )
} // ExpComp 컴포넌트 
