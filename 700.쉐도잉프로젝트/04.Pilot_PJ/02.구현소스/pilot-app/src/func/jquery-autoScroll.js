...
********************************************/
  function movePg() {
    ...
    $("html,body").stop().animate({scrollTop: $(window).height() * pno + "px",},700,"easeInOutQuint",actPage);
  ...
  *******************************************************/
  function initSet() {
    // 1. 초기화하기 
    // 대상 : .imgc
    $('.imgc').css({transform:'rotate(45deg)',transformOrigin:'-10% -10%',opacity:0,transition:'1s ease-in-out'}); // css
    // 대상 : .txtc a
    $('.txtc a').css({transform:'rotate(45deg)',transformOrigin:'-100% -100%',opacity:0,transition:'1s ease-in-out .5s',display:'inline-block'}); // css
 ...
 function actPage(){
  ...
  if(pno!=0 || pno!=4){
    ...
    $('.page').eq(pno).find('.imgc, .txtc a').css({transform:'rotate(0deg)',opacity:1}); // css
  } // if 
