...
export function VidIntro(props) {
  ...
  const linkCode = (data) => {
    return (
      <>
       {data.split("*")[0]}  
       <a href={selData.link[1]} target="_black">
        {selData.link[0]}
       </a>{data.split("*")[1]}
      </>
    );
    ...
    <section className={"vidbox" + " " + props.cls}>
      <div>
        ...
        <div className="vb2">
          ...
          {/* 요약소개 : sum */}
          <p>
            {/* 특수문자(*)여부에 따라 처리 indexOf(문자열) -> 없으면 -1 리턴 */}
            {selData.sum.indexOf("*") == -1 ? selData.sum : linkCode(selData.desc)}
          </p>
          {/* 설명(링크포함) : desc */}
          <p>
            {/* 특수문자(*)여부에 따라 처리 indexOf(문자열) -> 없으면 -1 리턴 */}
