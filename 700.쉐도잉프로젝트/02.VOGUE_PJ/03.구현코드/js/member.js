...
form.logF input[type=password]`)
  ...
  .blur(function () {
     ...
     *****************************************/
        ...
        if(!vReg(cv,cid)){ // 아이디검사 불통과시 (!NOT)
            $(this).siblings('msg').text('영문자로 시작하는 6~20글자 영문자/숫자')
        } // if
        
        else{ // 통과시
            // 1. DB에 조회하여 같은 아이디가 있다면 '이미 사용중인 사용중인 아이디입니다.' 와 같은 메시지를 출력
            // 2. 만약 DB 조회하여 같은 아이디가 없다면 '멋진 아이디네요!' 와 같은 메시지 출력 
            // 여기서 우선은 DB조회 못하므로 통과시 메시지로 출력 
            // 메시지 띄우기 
            $(this).siblings('msg').text('멋진 아이디네요!')
        } // else 
