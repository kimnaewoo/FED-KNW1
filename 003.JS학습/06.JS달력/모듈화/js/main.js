...
// 다음달 달력 쓸 수 있게 이전/다음달 달력변경 내부함수를 생성자함수에 this 키워드로 등록하여 노출해야한다!
dc2.chgCalendar(1)


// 제이쿼리로 달력 컴포넌트를 사용할때 추가기능 구현하기 

// 1. 대상선정 : 달력 .calendar
const calBox = $('.calendar .main');
console.log(calBox);
 
// 2. 이벤트 설정하기 : mouseenter - 마우스가 달력박스 안에 들어갓을때 처리 
// on(이벤트명,함수) -> 별도의 이벤트메서드가 있다면 그것 사용 
// mouseenter(함수)
// calBox.on.('mouseenter',fn)
calBox.mouseenter(function(){
    // this 는 선택달력의 .main 박스 
    console.log('나야나',this);

    // 선택달력 하위 .date를 클릭하면 히든필드 날짜정보 읽어오기! 
    $(this).find('.date').click(()=>{
        // 제이쿼리는 value 대신에 val 메서드 사용 
        let myData = $(this).next().val();

        // 달력 인스턴스생성시 노출된 속성/메서드중 
        // addZero() - 한자리숫자앞에 0추가 기능 
        // week[] - 한글 요일변환 배열 
