...
// -> from 뒤의 파일명확장자사용해야함! -> SPA 모듈 개발환경에서는 .js, .jsx 등 확장자 생략가능 
// 텍스트 데이터
// import { mTitle,sTitle,personInfo,mvData } from "./text_data.js";


// 별칭으로 받기 as 별칭 => 별칭을 썻으면 별칭으로 사용해야함!
...
// default 로 내보낸것은 {변수} 형식으로 못받는다.

// import mTitle from "./text_data.js";
// import zzz from "./text_data.js";

//  console.log(zzz("헐"));
// console.log(dFn,mTitle,sTitle,personInfo,mvData);
console.log(msgFn);
/*************************************************** 
...
titBox.innerHTML = `
    ...
    <h3>${sTit}</h3>
`;
// (2) 내용 출력하기 : 이름과 나이를 소개하는 메시지 넣기
contBox.innerHTML = msgFn('공유',45);
contBox.innerHTML += msgFn('톰 행크스',59);
contBox.innerHTML += msgFn('안젤리나 졸리',48);
// 이름과 나이가 셋팅된 배열데이터를 이용하여 출력하기
pInfo.forEach(val=>contBox.innerHTML+=msgFn(val[0],val[1]));

// (3) 영화정보 뿌리기
// ol>li 형식으로 .mvpart 박스에 영화정보 를 구성한다.
// 데이터는 mv변0수 
