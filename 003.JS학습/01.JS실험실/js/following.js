...
function loadFn(){
    ...
    2. offsetX, offsetY
        -> 이벤트 대상이 기준
        -> 특정박스에 부모자격박스로 부터 위치를 사용할 경우

    3. pageX, pageY
        -> 전체 문서를 기준(스크롤 화면을 포함)
        -> 화면을 기준한 apsolute 포지션에서 주로 사용!
    4. screenX, screenY
        -> 모니터 화면을 기준
    ...
    ********************************************************/
    ...
    // 2. 이벤트 대상에 마우스 무브 이벤트가 적용될때 
    // 무버가 따라다니게 기능구현
    myBody.onmousemove = e => { // e - 이벤트전달변수
        // 2-1. 위치값 가져오기 (박스 중앙 위치로 보정)
        let posx = e.pageX - gap;
        let posy = e.pageY - gap;
        // let posy = e.clientY - gap; -> 만약 .mover가 fixed 포지션이면 브라우저 화면에서의 위치인 client를 사용한다!
        // 2-2. 무버에 위치값 적용하기
