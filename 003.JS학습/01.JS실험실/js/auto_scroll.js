...
/********************************************************************************************************** 
    [ 자동스크롤 기능정의 ]
    1. 스크롤바가 없는 상태에서 마우스휠 작동시 아래와 같이 기능구현
    (1) 휠 내림 : 다음페이지 이동
    (2) 휠 올림 : 이전페이지 이동
    ...
    -> wheel 이벤트 
    -> 마우스 휠 작동시 발생! 
    (이전 이벤트명 : mousewheel / DOMMouseScroll(firefox))
**********************************************************************************************************/
...
function wheelFn(e){ // 이벤트 전달변수
    ...
    sts_wheel = 1; // 잠금!
    setTimeout(()=>{sts_wheel=0},500); // 0.5초후 잠금해지
...
function chgMenu(){
    ...
    // 메뉴 li를 순회하여 해당순번(pg_num)에 .on넣기
    // 나머지는 .on 빼기
    ...
    // forEach()가 gnbList 와 indicList 를 각각 comFn에 전달한다
...
function touchStart(e){ // e - 이벤트 전달변수
    ...
    // 변경된 터치이벤트 담는 컬렉션 - changedTouches[0]
