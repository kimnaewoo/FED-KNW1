...
/************************************************************************ 
    ...
    1. 사용 이벤트 : scroll
    -> 스크롤 바가 있는 페이지에서 또는 부분박스에서 스크롤 바가 이동할때 발생하는 이벤트
    -> 주의 : wheel 이벤트와는 다르다. 무엇이? 
    -> 스크롤바가 이동하지 않아도 마우스 휠이 작동될때 발생한다.
    (wheel 이벤트는 모바일에서 사용이 불가하다.)

    2. 스크롤바 위치값 알아내기 : 세로방향 (Y축)
        2-1. window.scrollY (IE6~11지원안함)
        2-2. document.scrollingElement.scrollTop
        2-3. document.documentElement.scrollTop
        2-4. document.querySelector('html').scrollTop
        -> 가로방향 스크롤바는 Y대신 X라는 문자를 사용!

    3. 스크롤 등장 대상요소의 보이는 화면에서의 top값
        getBoundingClientRect().top
        -> 보이는 화면상단을 기준으로 이것보다 위로 갈경우 마이너스 값을 리턴한다.
        -> 기준: 보이는 화면크기를 기준하면 된다.
        -> window 화면 전체: window.innerHeight
