...
/****************************************************************************************************************************************************************************** 
        [ 페럴렉스 기능구현 하기 ]
        1. 정의 : 스크롤 작동시 같은 방향으로 요소가 다른속도를 가지고 움직이므로 사용자가 공간감을 느낄 수 있게 하는 구현방법
        2. 방법 
            1) 범위 : 요소가 화면에 등장하여 보일동안 스크롤될때 이동한다.
            2) 움직일 크기설정이 필요하다.
            3) 범위체크를 위한 JS 메서드를 사용한다. -> getBoundingClientRect().top
        3. 이벤트 : scroll
        4. 패럴렉스 대상 : 특정클래스 지정
            1) 글자박스 대상 : .txt 
            2) 아이콘이미지 대상 : .icon 
...
function moveEl(elPos,ele,setH){
    ...
    // [ 패럴렉스 범위 : 윈도우높이값 ~ 0 ]
    // 화면에서 완전히 사라질때 범위는 0보다 작다(약간의 마이너스)
    if(elPos<winH && elPos>0){
        // 1. 위치이동값 계산 
        // 실제이동값 = 지정한범위 - (위치값*지정한범위 / 전체범위) 
        let x = setH - (elPos*setH / winH);
        console.log('x:',-x);

        // 2. 해당요소의 위치값 이동 CSS반영하기
        // Y축 이동시 윗쪽방향은 마이너스 이다. -x
    ...
    /****************************************************** 
        ...
        ((비례식으로 실제 이동값 알아내기))
        전체범위 : 위치값 = 지정한범위 : 실제이동값 
        실제이동값 = 위치값*지정한범위 / 전체범위 

        -> 그.런.데... Y축 위치이동은 처음에 0부터 서서히 커지므로 이동수치값은 지정한범위에서 실제이동값을 빼야한다.
