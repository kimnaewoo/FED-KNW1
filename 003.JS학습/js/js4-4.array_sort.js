...
function searchingFn(){
    ...
    // 3. 다중값 return LIKE 검색 : 원본데이터 (list2)로 검색
    // filter() + indexOf() 사용!
    let res = list2.filter(v=>{
        // v[객체속성명] -> v[cta]
        // -> cta변수에 idx/tit/cont 중 하나 
...
dFn.addEvt(dFn.qs('.sample'),'click',()=>{
    ...
    // 2. find() 메서드 LIKE 검색하기 : 데이터 일부분을 통해서 찾음 
    // indexOf() 결과가 -1이 아니면 내용이 있으므로 처리, find() 의 특성상 처음 만나는 데이터 하나만 리턴함 !
