...
function localSFn(){
    ...
    // -> 객체를 생성하여 로컬 스토리지에 넣기
        ...
        // 'minfo'값이 '[]'배열 초기화 값이면, makeObj() 호출 ! 
...
function bindData(){
    ...
    if(localData){ // null이 아니면 true!
        ...
        bindCode = localData.map((v,i)=>`
            <tr>
                ...
                <td>${v.tit}</td>
                <td>${v.cont}</td>
                <td class="del-link"><a href="#" data-idx=${i}">×</a></td>
            </tr>
    ...
    // 5. 화면출력 후 지우기 셋팅하기 
    dFn.qsa('.board .del-link a').forEach(ele=>dFn.addEvt(ele,'click',()=>delRec(ele.getAttribute('data-idx'))));
...
function insData(){
    ...
    // 2. 만약 하나라도 비었다면 돌아가! 
    // trim() 앞뒤공백 제거 -> 스페이스바만 쳐도 불통과! 
    if(tit.trim()=='' || cont.trim()==''){
        alert('입력데이터가 없습니다. 모두 입력하십시오.')
        return;
    } // if 
    ...
    // 3. 입력처리하기 
    // 3-1. 로컬스토리지 데이터 가져오기 : minfo
    let orgData = localStorage.getItem('minfo');

    // 만약 minfo 로컬쓰가 null 이면 빈 배열로 생성하기! 
    if(!orgData){
        // 빈 배열로 생성하기 
    ...
    // 3-3. 자동증가번호 처리하기 
    // 배열을 오름차순으로 정렬하여 맨끝 배열데이터에 idx값을 읽어온후, 숫자화 하여 +1 처리함
    // 3-3-1. 배열 오름차순 처리 : 
...
function delRec(idx){
    ...
    // 3. 특정 데이터 배열항목 삭제하기 
    // splice(순번,갯수) -> 특정순번부터 몇개지움 
    // 여기서는 1개삭제 이므로 splice(순번,1)
    // confirm(메시지) -> 확인,취소 중 확인 클릭시 true 리턴한다. (취소는 false)
    if(confirm('정말정말정말정말로 지우시게요????')){
        orgData.splice(idx,1);
        // console.log('제거후 배열:',orgData);
...
function setMod(){
    ...
    /**************************************************************************** 
        ...
        -> 값이 있는 배열을 만나면 바로 순회를 마친다. 배열값을 변수에 저장함!
    ****************************************************************************/
